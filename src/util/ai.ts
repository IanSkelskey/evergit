...
class OllamaProvider implements ModelProvider {
    ...
    async createCompletion(systemPrompt: string, userPrompt: string, model: string): Promise<string | null> {
        ...
        try {
            ...
            if (responseData.choices && Array.isArray(responseData.choices) && responseData.choices[0]?.message?.content) {
            ...
            if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
                print('error', 'Ollama request timed out. The model might be loading or the server is slow. Try increasing the timeout.');
            } else if (error.response?.status === 403) {
                print('error', 'Authentication failed. Please ensure OLLAMA_API_KEY environment variable is set correctly.');
                console.error('Response data:', error.response.data);
            } else if (error.response?.status === 404) {
                print('error', `Model '${model}' not found. Please check if the model is available on your Ollama server.`);
