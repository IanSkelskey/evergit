...
function setConfig(key: keyof Config, value: string): void {
    ...
    if (key === 'provider') {
        if (value === 'openai' || value === 'openwebui') {
            config[key] = value as 'openai' | 'openwebui';
        } else {
...
function isValidKey(key: string): key is keyof Config {
    return ['name', 'email', 'provider', 'openaiModel', 'openwebuiModel', 'openwebuiBaseUrl'].includes(key);
}
